...
function generateTexture(card: Card) {
  ...
  return new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(canvas), color });
}
...
export class CardContainer extends THREE.Group {
  ...
  constructor(public game: Game, public card: Card) {
    ...
    loader.load("/assets/models/card.glb", (gltf) => {
      ...
      g1.rotateZ(!card.open ? 0 : Math.PI);
      this.backMesh = (g1.children[0] as THREE.Mesh);
      this.frontMesh = (g1.children[1] as THREE.Mesh);
